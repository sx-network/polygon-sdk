...
type forkManagerInterface interface {
	...
	GetCustomContractAddress(uint64) (types.Address, error)
}
...
func (i *backendIBFT) getCustomContractAddressFromCurrentFork(height uint64) types.Address {
	customContractAddress, err := i.forkManager.GetCustomContractAddress(height)
	if err != nil {
		i.logger.Error("could not get customContractAddress from current fork", "error", err)
