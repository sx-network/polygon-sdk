...
// ValidatorSetToAddressArray converts Validators collection to []types.Address
func ValidatorSetToAddressArray(validators Validators) []types.Address {
	addresses := make([]types.Address, validators.Len())

	for idx := 0; idx < validators.Len(); idx++ {
		validator := validators.At(uint64(idx))
		addresses[idx] = validator.Addr()
	}
